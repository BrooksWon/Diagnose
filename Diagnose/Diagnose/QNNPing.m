...
- (instancetype)initWithCode:(NSInteger)code
           ...
           standardDeviation:(NSTimeInterval)standardDeviation;
...
- (NSString *)description {
    if (_code == 0 || _code == kQNNRequestStopped) {
        return [NSString stringWithFormat:@"%d packets transmitted, %ld packets received, %f packet loss time %fms\n round-trip min/avg/max/standardDeviation = %.3f/%.3f/%.3f/%.3f ms",
                                          (int) (_count + _loss),
                                          (long) _count,
                                          (double) _loss * 100 / (_count + _loss),
                                          _totalTime,
                                          _minRtt,
                                          _avgRtt,
                                          _maxRtt,
...
static BOOL isValidResponse(char *buffer, int len, int seq, int identifier) {
    ...
    return receivedChecksum == calculatedChecksum && icmpPtr->type == kQNNICMPTypeEchoReply && icmpPtr->code == 0 && OSSwapBigToHostInt16(icmpPtr->identifier) == identifier && OSSwapBigToHostInt16(icmpPtr->sequenceNumber) <= seq;
}

@interface QNNPing ()
@property (strong, readonly) NSString *host;
@property (weak, readonly) id <QNNOutputDelegate> output;
@property (copy, readonly) QNNPingCompleteHandler complete;

@property (assign, readonly) NSInteger interval;
...
- (int)sendPacket:(ICMPPacket *)packet sock:(int)sock target:(struct sockaddr_in *)addr {
...
- (int)ping:(struct sockaddr_in *)addr seq:(uint16_t)seq identifier:(uint16_t)identifier sock:(int)sock ttl:(int *)ttlOut size:(int *)size {
    ...
    ssize_t bytesRead = recvfrom(sock, buffer, kQNNPacketBufferSize, 0, (struct sockaddr *) &ret_addr, &addrLen);
    if (bytesRead < 0) {
        err = errno;
    } else if (bytesRead == 0) {
        err = EPIPE;
...
- (QNNPingResult *)buildResult:(NSInteger)code
    ...
    if (code != 0 && code != kQNNRequestStopped) {
        return [[QNNPingResult alloc] initWithCode:code max:0 min:0 avg:0 loss:1 count:1 totalTime:time standardDeviation:0];
    }
    ...
    NSTimeInterval standardDeviation = sqrt(avg2 - avg * avg);

    return [[QNNPingResult alloc] initWithCode:code max:max min:min avg:avg loss:loss count:count totalTime:time standardDeviation:standardDeviation];
}
...
- (void)run {
        ...
        if (host == NULL || host->h_addr == NULL) {
            ...
            if (_complete != nil) {
                dispatch_async(dispatch_get_main_queue(), ^(void) {
                    QNNPingResult *result = [[QNNPingResult alloc] initWithCode:-1006 max:0 min:0 avg:0 loss:0 count:0 totalTime:0 standardDeviation:0];
                    _complete(result);
                });
            }
            return;
    ...
    if (_complete) {
        ...
        QNNPingResult *result = [self buildResult:code
                                        durations:durations
                                            count:index - loss
                                             loss:loss
...
+ (instancetype)start:(NSString *)host
             ...
             interval:(NSInteger)interval
                count:(NSInteger)count {
    QNNPing *ping = [[QNNPing alloc] initWithHost:host output:output complete:complete interval:interval count:count];
