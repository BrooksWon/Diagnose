...
- (instancetype)initWithCode:(NSInteger)code max:(NSTimeInterval)maxTime min:(NSTimeInterval)minTime avg:(NSTimeInterval)avgTime count:(NSInteger)count;
...
@property (copy, readonly) QNNTcpPingCompleteHandler complete;
...
- (void)run {
        ...
        if (host == NULL || host->h_addr == NULL) {
            ...
            if (_complete != nil) {
                dispatch_async(dispatch_get_main_queue(), ^(void) {
                    _complete([self resultAsCode:-1006 durations:nil count:0]);
                });
            }
            return;
        }
    ...
    if (_complete) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^(void) {
            _complete([self resultAsCode:code durations:intervals count:index]);
        });
    }
...
- (QNNTcpPingResult *)resultAsCode:(NSInteger)code durations:(NSTimeInterval *)durations count:(NSInteger)count {
    if (code != 0 && code != kQNNRequestStopped) {
        return [[QNNTcpPingResult alloc] initWithCode:code max:0 min:0 avg:0 count:1];
    }
    ...
    return [[QNNTcpPingResult alloc] initWithCode:code max:max min:min avg:avg count:count];
}
...
+ (instancetype)start:(NSString *)host
             ...
             complete:(QNNTcpPingCompleteHandler)complete;
{
    QNNTcpPing *tcpPing = [[QNNTcpPing alloc] initWithHost:host port:port output:output complete:complete count:count];
